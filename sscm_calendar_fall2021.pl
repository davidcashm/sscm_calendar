...
# sscm_calendar_2020.pl
# Copyright (C) 2007-2020 David Cashman and Ari Meir Brodsky
...
my $weekday_shacharis = "6:45" . $separator . "7:30" . $separator . "8:30";
...
my $rosh_chodesh_and_fast_day_shacharis = $START_BOLD . "6:30" . $END_BOLD . $separator . "7:30" . $separator . "8:30";
...
my $slichos_before_RH = "6:15" . $separator . "7:00" . $separator . "8:00";
my $slichos_after_RH = "6:05" . $separator . "7:00" . $separator . "8:00";
...
warn "Check 1:00 Mincha for fast days etc. \n";
...
my $shofar = "(approx.) 10:45" . $separator . "1:30";		# changed to 11:00 by Rabbi Felder's request in 2015, then to 10:40 in 2019 (was 10:30 in 2018 by e-mail notice), in 2021 changing to 10:45 and adding 1:30 after davening
...
"Fast ends",
"Megillas Esther",
"Dor L'dor", # this is for when Avos UBanim is on Motzai Shabbos.
"Learning Program",
...
#                ($day->{Emon} == 3 && $day->{Eday} < 14))        # until March clock change
      ...
      #my $aub_start = is_earlier($motz, "6:01") ?
		...
		# RULES FROM 2013, sort of:
                ...
                {
                    $day->{to_print}->{"Dor L'dor"} = $aub_start . "-" . $aub_end;
                }
                ...
                #{
                #    $day->{to_print}->{"Dor L'dor"} = "7:00-8:00";
                #}
                #elsif (is_earlier($motz,"6:31"))
                #{
                #    $day->{to_print}->{"Dor L'dor"} = "7:15-8:15";
                #}
                #elsif (is_earlier($motz,"6:46"))
                #{
                #    $day->{to_print}->{"Dor L'dor"} = "7:30-8:30";
                #}
                #else
                #{
                #    # Never start later than 7:30
                #    # $day->{to_print}->{"Dor L'dor"} = "7:45";
                #}
            }
...
#         if (!$renovation)
		...
		delete $day->{to_print}->{"Likras Shabbos for Boys"};
         ...
         else
         {
            ...
            $day->{to_print}->{Shacharis} = $early_shacharis . $separator . $vasikin . $separator . "7:30" . $separator . "8:30" . $BOLD;
...
#    print "$day->{Eyear}/$day->{Emon}/$day->{Eday} new value was " . $day->{to_print}->{"Fast begins"} . "\n";
         ...
         my $sunset = get_sunset($day);
         $day->{to_print}->{Sunset} = $sunset;
         my $mincha = $BOLD . round_down_to_5_minutes(subtract_minutes($sunset,30));
	if ($day->{day_of_week} !~/Fri/ && !is_during_DST($day))
	{
		$mincha = $early_sunday_mincha_winter . $separator . $mincha;
	}
	else
	{
		$mincha = $mincha . $only;
	...
	$day->{to_print}->{Mincha} = $mincha;
         ...
         if ($day->{day_of_week} ne "Fri")  # AB added this condition
         {
            ...
            {
                $maariv .= $separator . "8:15" . $separator . "9:30";
            }
            else
            {
                $maariv .= "$BOLD$only";
...
#      }
      ...
      if ($erev->{day_of_week} eq "Sat")
      {
         ...
         delete $erev->{to_print}->{"Dor L'dor"};
      }
      else
      {
...
#   }
   ...
   #### MAY NEED TO MANUALLY RESIZE BOTTOM ROW!!!!!
    ...
    {
    	$footnote_text = $footnote_text . $COVID_note . chr(10) . chr(10);
	warn "May need to resize bottom row manually because of YMM note or COVID note!";
    }
