...
# sscm_calendar_2023.pl
# Copyright (C) 2007-2023 David Cashman and Ari Meir Brodsky
...
my $YMM_is_uncertain = 0; # in 5780 the time is being tested
...
my $default_hebcal_file = "all_calendar_years.txt";   # "dates.txt";
...
my $COVID_note = "All times are subject to change due to COVID-19 developments." . chr(10) . 
	"Please refer to the weekly ebulletin from SSCM.";

my $change_note = "Scheduled times may occasionally change due to unforeseen circumstances." . chr(10) . 
	"Please consult weekly bulletin for updates.";
...
my $endash = chr(0x2013);
my $text_endash = "--";
my $emdash = chr(0x2014);
my $text_emdash = "---";
my $non_breaking_hyphen = chr(0x2011);	# haven't used this one yet
...
my $breakable_separator = $nbsp . "/ ";	# to use when there are four times, allowing a line-break
...
# CHANGING IN 2020 DURING COVID; NOT SURE HOW MUCH OF THIS IS PERMANENT:
my $weekday_shacharis = "6:45" . $breakable_separator . "7:00" . $separator . "7:30" . $separator . "8:30";
my $early_shacharis = $weekday_shacharis;
my $late_shacharis = $weekday_shacharis;
my $isru_chag_shacharis = $weekday_shacharis;
my $sunday_shacharis = "7:30" . $separator . "8:00" . $separator . "8:30";
#my $sunday_shacharis = "7:30" . $separator . "8:30";
my $holiday_shacharis_no_change = 0;	# from 2022, holiday Shacharis is same as other weekdays.  But then in 2023 adding 7:00 on weekdays but NOT holidays.
#my $holiday_shacharis = $BOLD . $sunday_shacharis;  # for public holidays
#my $holiday_shacharis = $BOLD . "7:30" . $separator . "8:30";
# starting 2023, same as weekday times but without the 7:00, BUT WE NO LONGER USE THIS CONSTANT - instead we just dynamically remove the 7:00.
#my $holiday_shacharis = "6:45" . $separator . "7:30" . $separator . "8:30";
my $shabbos_shacharis = "8:00" . $separator . "9:00" . $separator . "9:15";  # 2022 change
# my $shabbos_shacharis = "7:30" . $separator . "9:00";  # "8:00" . $separator . "9:00"; # adding 7:30 in 2021
my $zachor_time = "(approx.) 9:20" . $separator . "10:20";  # . $separator . "10:35";  # 1 hour 20 minutes after each Shacharis, but in 2022 they said not to list the last one
# my $zachor_time = "(approx.) 8:50" . $separator . "10:20";  # . $separator . "11:15";  # no longer after davening
my $yom_tov_shacharis = "8:00" . $separator . $START_BOLD . "9:00$END_BOLD";  # returning to this in 2022?
#  my $yom_tov_shacharis = $shabbos_shacharis;   # "9:00";
my $shavuos_shacharis = $BOLD . "9:00"; # also add vasikin
# my $yizkor_time = "9:00" . $separator . "10:30 (approx.)";    # for the 3 regalim, not Yom Kippur (first time is hashkama)
#my $yizkor_time = "10:30 (approx.)";    # for the 3 regalim, not Yom Kippur
my $rosh_chodesh_and_fast_day_shacharis = $START_BOLD . "6:30" . $END_BOLD . $breakable_separator . "7:00" . $separator . "7:30" . $separator . "8:30";
my $rosh_chodesh_shacharis = $rosh_chodesh_and_fast_day_shacharis;
my $fast_day_shacharis = $rosh_chodesh_and_fast_day_shacharis;
my $chanuka_shacharis = $weekday_shacharis;

my $slichos_erev_RH_sunday = "6:30" . $separator . "6:45";   # includes public holiday
my $slichos_erev_RH_weekday = "5:30" . $separator . "6:45";		# added 7:00 in 2022; in 2023 changing to 6:45
my $slichos_erev_YK_weekday = "6:30" . $separator . "7:15" . $separator . "8:15";
my $slichos_erev_YK_sunday = "7:15" . $separator . "7:45" . $separator . "8:15";
my $slichos_sunday = "7:00" . $separator . "7:30" . $separator . "8:10";
my $slichos_public_holiday = "7:00" . $separator . "8:10";	# Maybe this one isn't used any more.
my $slichos_before_RH = "6:15" . $separator . "7:00" . $separator . "8:10";
my $slichos_tzom_gedalia = "6:05" . $separator . "7:00" . $separator . "8:10";
my $slichos_after_RH = "6:15" . $separator . "7:00" . $separator . "8:10";

#warn "NEED TO REPROGRAM SLICHOS!!!!!!!!!  \n";
#warn "Chol hamoed, Rosh Chodesh, fast days, and Purim got messed up! \n";
#warn "They depend on checking for sunday_shacharis, but weekday_shacharis now contains sunday_shacharis ! \n";
warn "Need to check Yom tov also! \n";

warn "Check Zachor times";

# PREVIOUS VERSIONS BEFORE COVID:
...
my $womens_avos = "5:30";
my $pre_slichos_event = "11:45"; # was 11:45 originally; changed to 12:00 in 2018, though the e-bulletin said 12:15, then 11:50 in 2021
#  my $first_slichos = "1:00 a.m.";  # NOT NECESSARILY 1:00 SINCE 2021
...
warn "Check 1:00/2:00 Mincha for fast days etc. \n";

my $shabbos_mincha_gedola = "2:00";
...
my $neilas_hachag_mincha_length = 25;   # need time to wash and start eating neilas hachag
...
my $shabbos_no_ss_early_mincha = $shabbos_mincha_gedola;  #"2:30";    # early Mincha so people can eat SS at home afterward on erev yom tov or first day of YT (or erev Tisha b'Av?) --- was 2:30, but no reason to differentiate from regular weeks
...
my $shofar = "(approx.) 10:55" . $separator . "1:30";		
	# changed to 11:00 by Rabbi Felder's request in 2015, then to 10:40 in 2019 (was 10:30 in 2018 by e-mail notice), 
	# in 2021 changing to 10:45 and adding 1:30 after davening
	# for 2023 we changed it to 10:55
...
#my $purim_mincha_DST = "2:00" . $separator . $purim_mincha;
#my $purim_mincha_standard = "1:00" . $separator . $purim_mincha;
my $motz_purim_maariv = $BOLD . "8:15" . $separator . "9:30";		# fixing in 2023
#my $motz_purim_maariv = $BOLD . "8:45" . $separator . "9:30" . $separator . "9:45";

# minimum lengths before sunrise for Vasikin
my %vasikin_length = (
    "14,Adar" => 23,
    "14,Adar II" => 23,
    "21,Tishrei" => 38,  # long pesukei dezimra but no Nishmas (added 5 minutes in 2016; increasing this again in 2023)
...
#  No longer using it for that purpose, as there is always 8:30 Shacharis now.
#  But we can use this for the 2:00 Mincha that started during bein hazmanim in 2023.
...
my $siyum_time = (   $COVID ? "" : "& Breakfast ") . "after each minyan";
...
my %omit_during_COVID = 
(
"Mishnayos" => 1,
"Chumash Shiur" => 1,
#"Yeshivas Mordechai HaTzaddik" => 1,
#"Zachor" => 1,
#"Sisterhood Tea" => 1,
"Avos UBanim " => 1, # when it's Shabbos afternoon.  Extra space is similar to the Megillas Esther hack.
"Likras Shabbos for Boys" => 1,
"Shiurim" => 1,
"Navi Shiur" => 1,
"Tehillim" => 1, # for Rosh HaShana and Yom Kippur
"Neilas Hachag" => 1,
#"Shiur" => 1,   # This used to be a Nach shiur until 5768, so references in the variable names and comments are still to Nach,
            # but the display is changed to Shiur in 5770.
"Learning Program for Boys" => 1,
);


my %omit_when_no_shiurim_listed = 
(
"Mishnayos" => 1,
"Chumash Shiur" => 1,
"Avos UBanim " => 1, # when it's Shabbos afternoon.  Extra space is similar to the Megillas Esther hack.
...
"Chatzos",
"Yizkor",
#"Mincha gedola",	# just for testing
"Sisterhood Tea",
...
"Fast ends",
"Megillas Esther",
"Avos UBanim", # this is for when Avos UBanim is on Motzai Shabbos.  From 2020-2022 it was called "Dor L'dor".
"Learning Program",
"Learning Program for Boys",
"Tickets",
"Melava Malka Hisorerus",
);
...
sub read_basic_calendar($)
{
   ...
   while (<IN>)
   {
      ...
      elsif ($line =~ /(\d+)\/(\d+)\/(\d+)\s+(\S\S\S),\s+(Parashat|Parshas)\s+(\S+.*)$/)
      {
         ...
         $parasha =~ s/Beshalach/Beshalach $emdash Shira/;    # adding Shira starting for 5773
...
#         $maariv = get_normal_maariv($day, $mincha);
	  ...
	  #  No longer need this as it's done already in set_mincha_maariv_for_week
      #if ($day->{day_of_week} !~/Fri|Sat/ && !is_during_DST($day))
      #{
      #	$day->{to_print}->{Mincha} = get_earliest_mincha($day) . $separator . $mincha;
      #}
	  
	  #  2:00 Mincha added on summer Sundays starting in 2023.
	  #  AMB noticed that for some weeks in July,  Mincha Gedola gets to 2:02, so need to delay Mincha to 2:05.
	  if ($day->{day_of_week} =~/Sun/ && is_during_DST($day))
		  {
			  $day->{to_print}->{Mincha} = get_earliest_mincha($day) . $breakable_separator . $mincha;
		  }
...
#		$day ->{to_print}->{"Likras Shabbos for Boys"} = subtract_minutes($mincha, $likras_shabbos_length);
	...
	if (!is_during_DST($day))
	{
		$day->{to_print}->{"Learning Program for Boys"} = "8:30" . $endash . "9:30";
	}
      }
...
#         }
	...
	#  Navi shiur started in summer 5774.  Adding in calendar for 5775:
...

         {
            ...
            {
                my $avos = add_minutes($day->{to_print}->{Mincha},55);	# 5 minutes earlier as requested in 2023
...
#                ($day->{Emon} == 3 && $day->{Eday} < 14))        # until March clock change
		...
		# In 2023 Amiel and Rav Yehoshua wrote to me:  
		#  "avos ubonim continues all the way until we change the clocks, 1 hour after shabbos rounded up to the nearest 5."
		#	I assume it still doesn't need to start before 7:00 on the earlier days.
      ...
      #my $aub_start = is_earlier($motz, "6:01") ?
		...
		#}
                ...
                my $aub_start = add_minutes (round_up_to_n_minutes ($motz, 5), 60);
...
#                if (is_earlier($aub_start, "7:31"))	# no longer using this condition, as of 2023
                {
                    $day->{to_print}->{"Avos UBanim"} = $aub_start . $endash . $aub_end;
                }
                ...
                #{
                #    $day->{to_print}->{"Avos UBanim"} = "7:00" . $endash . "8:00";
                #}
                #elsif (is_earlier($motz,"6:31"))
                #{
                #    $day->{to_print}->{"Avos UBanim"} = "7:15" . $endash . "8:15";
                #}
                #elsif (is_earlier($motz,"6:46"))
                #{
                #    $day->{to_print}->{"Avos UBanim"} = "7:30" . $endash . "8:30";
                #}
                #else
                #{
...
#                if (!defined($early_mincha))
                ...
                $day->{to_print}->{"Avos UBanim "} = "5:00" . $endash . "6:00";
...
#                }
         ...
         if (defined($special_shabbos))
         {
            ...
            $parasha .= " $emdash $special_shabbos";
...
#            $day->{to_print}->{Parasha} = $day->{parasha};
            add_prenotes($day, $parasha);    # Now this will include the Special Shabbos listing (maftir).  Also, it was already BOLDed when stored.
         }
         else
         {
...
#         }
	...
	# In fall 2020 (COVID lockdown) added 2:00 Shabbos Mincha
	# It can stay at 2:00 even if Mincha gedola is slightly later, because "It doesnt always start exactly on time and it's a longer davening anyways"
	#  (e-mail from Amiel on Aug. 7, 2023)
	$day->{to_print}->{Mincha} = $shabbos_mincha_gedola . $separator . $day->{to_print}->{Mincha};
...
sub apply_advanced_rules()
{
   ...
   # And even worse now (2020) because Sunday Shacharis times are a subset of weekday Shacharis times,
   # so checking that the string includes sunday_shacharis leads to wrong results!
   modify_public_holidays();
   warn "As of changes for 5784, public holiday Shacharis times may not be correct in all cases (check fast days), 
					and Slichos on public holidays hasn't been fixed. \n";
...
#          $day->{to_print}->{Slichos} eq "1:00 a.m." &&
	...
	if((!$COVID || !defined($omit_during_COVID{$item}))  &&
		(!$no_shiurim || !defined($omit_when_no_shiurim_listed{$item})))
         {
...
sub dump_calendar()
{
   ...
   foreach my $day (@list_of_days)
   {
	  ...
	  if (defined($day->{perek}))
		  {
			  print OUT "Avos " . get_text_safe($day->{perek}) . "\n";
		  }
...
sub get_text_safe($)
{
        ...
        $item =~ s/$endash/$text_endash/;
        $item =~ s/$emdash/$text_emdash/g;
        return $item;
}
...
sub check_tefillin_time($$)
{
    ...
    if ($shacharis ne "TBA" && is_earlier($shacharis,subtract_minutes($sunrise, 45)))
    {
...
sub set_mincha_maariv_for_week($)
{
    ...
    if ($thursday_index > $#list_of_days)
    {
		...
		$latest_earliest_mincha = "UNKNOWN";
        $mincha = "UNKNOWN";
        $maariv = "UNKNOWN";
    ...
    else
    {
        ...
        for my $index ($sunday_index .. $thursday_index)
        {
			...
			my $earliest_mincha_today = get_earliest_mincha($list_of_days[$index]);
            if (!is_time($sunset_today))
            {
                croak "Error: No valid sunset defined for day $index: $sunset_today\n";
            }
            if (!is_time($earliest_mincha_today))
            {
                croak "Error: No valid sunset defined for day $index: $earliest_mincha_today\n";
            }
            ...
            if (!defined($earliest_sunset))
            {
				...
				$latest_earliest_mincha = $earliest_mincha_today;
            }
            else
            {
				...
				if (is_earlier($latest_earliest_mincha,$earliest_mincha_today))
				{
					$latest_earliest_mincha = $earliest_mincha_today;
				}
            }
        }
        if (!defined($earliest_sunset) || !defined($latest_sunset) || !defined($latest_earliest_mincha))
        {
        ...
        if (is_earlier($early_weekday_mincha,subtract_minutes($primary_mincha,5)))	# in 2023 they said to remove 8:15 Mincha when primary Mincha is at 8:20.
        {
            $mincha = $even_earlier_weekday_mincha . $separator . $early_weekday_mincha . $separator . $primary_mincha;
        }
        elsif (is_earlier(add_minutes($even_earlier_weekday_mincha,5),$primary_mincha))	# no 6:00 Mincha if primary_mincha is at 6:05
        {
		...
		if (!is_during_DST($day))
		{
			$mincha = $latest_earliest_mincha . $separator . $mincha;
		}
...
#        }
	...
	# in 5781 adding 8:15 maariv in winter:
	# from instructions in 2022 it continues until the main Maariv gets as late as 8:15
	# but from clarification by e-mail from RYF on Sept. 5, 2022, it should start only when main Maariv is 8:00 or earlier.
	#  Not sure whether this was meant to apply to ending also.
	# in 2022 (or maybe already late 2021) adding Maariv at 9:45,
	# so there are 4 maarivs and we need a non-breaking separator in the list.
#	if (!is_during_DST($day))
	if (is_earlier($main_maariv, "8:01"))
	{
		$maariv = $main_maariv . $separator . "8:15" . $breakable_separator . $late_maariv . $separator . "9:45";
		# $late_maariv = "8:15" . $separator . $late_maariv;
	}
	else
	{
		$maariv = $main_maariv . $separator . $late_maariv . $separator . "9:45";
	}
    }
...
sub remove_extra_maarivs($)
{
	my ($day) = @_;
	my $maariv = $day->{to_print}->{Maariv};
	if (defined($maariv))
	{
		$maariv =~ s/($separator)9:45//;
		$maariv =~ s/($separator)8:15//;
		$day->{to_print}->{Maariv} = $maariv;
	}
}
...
sub set_holiday ($$)
{
    ...
    {
        my $day = $list_of_days[$idx];
        if(!$holiday_shacharis_no_change)
        {
			my $shacharis = $day->{to_print}->{Shacharis};
			$shacharis =~ s/($breakable_separator)7:00//;
			$day->{to_print}->{Shacharis} = $shacharis;
#        		$day->{to_print}->{Shacharis} = $holiday_shacharis;
        		# delete $day->{to_print}->{"Earliest Tefillin"};   # still have 6:45 Shacharis on holidays
        }
...
#        	$day->{to_print}->{Mincha} = $early_sunday_mincha_winter . $separator . $day->{to_print}->{Mincha};
        ...
        if (is_during_DST($day) && $day->{day_of_week} ne "Fri")
        {
        	$day->{to_print}->{Mincha} = get_earliest_mincha($day) . $breakable_separator . $day->{to_print}->{Mincha};
        }
		...
		add_prenotes($day, $BOLD . "Public holiday");
...
sub modify_rosh_chodesh()
{
   ...
   while ($day_idx <= $#list_of_days)
   {
      ...
      if ($heb_day == 1 || $heb_day == 30)
      {
		...
		{
			# add_prenotes($day, "Ulchapparas Pasha");
			# In 5782 R' Yehoshua didn't like the bold as it appeared to be an extension of the "Rosh Chodesh...'' title.
			# Then in 2023 they told me to omit it entirely.
		}
...
sub compute_shma ($$)
{
   ...
   return $ma_zksh . $separator . $gra_zksh;
}

sub compute_mincha_gedola_lechumra ($$)
{
   my ($sunrise,$sunset) = @_;

   my $minutes_per_shaa = minutes_per_shaa($sunrise,$sunset);
#   my $chatzos = add_minutes($sunrise, $minutes_per_shaa*6);
   my $mincha_gedola_lechumra = ($minutes_per_shaa > 60)	?	add_minutes($sunrise, $minutes_per_shaa*6.5 +1)	:	add_minutes($sunrise, $minutes_per_shaa*6 + 30);
   #  Adding one minute to account for rounding up rather than down as it's an earliest cutoff rather than a latest.
   #  No good reason to do it only in the summer case, except that it more closely matches the known charts this way.
   #  Needs to be investigated further.
   return $mincha_gedola_lechumra;
}

sub compute_ikkar_mincha_gedola
{
   my ($sunrise,$sunset) = @_;

   my $minutes_per_shaa = minutes_per_shaa($sunrise,$sunset);
#   my $chatzos = add_minutes($sunrise, $minutes_per_shaa*6);
   my $ikkar_mincha_gedola = add_minutes($sunrise, $minutes_per_shaa*6.5 +1);
   #  Adding one minute to account for rounding up rather than down as it's an earliest cutoff rather than a latest.
   return $ikkar_mincha_gedola;
}

sub get_earliest_mincha($)
{
	# getting earliest reasonable Mincha time, which may or may not be scheduled on a given day
	#  In 2023, they moved the 1:00 Mincha to 1:05, but then decided (e-mail on Aug. 17, 2023) to keep it at 1:00,
	#  but 2:00 needs to be moved to 2:01 or 2:02 as necessary.
	my ($day) = @_;
	my $earliest_mincha = (is_during_DST($day))	?	"2:00"	:	"1:00";
	my $ikkar_mincha_gedola = compute_ikkar_mincha_gedola(get_sunrise($day),get_sunset($day));
	$earliest_mincha = (is_earlier($earliest_mincha, $ikkar_mincha_gedola))	?	$ikkar_mincha_gedola	:	$earliest_mincha;

#	my $mincha_gedola_lechumra = compute_mincha_gedola_lechumra(get_sunrise($day),get_sunset($day));
#	if (is_earlier($earliest_mincha, $mincha_gedola_lechumra))
#	{
#		$earliest_mincha = add_minutes($earliest_mincha, 5);
#	}
#	warn "Mincha gedola lechumra: " . $mincha_gedola_lechumra . "; Earliest Mincha: " . $earliest_mincha;


	return $earliest_mincha;
}

sub test_getting_earliest_mincha()
{
	foreach my $day (@list_of_days)
	{
		if ($day->{idx} > 0)
		{
			my $mincha_gedola_lechumra = compute_mincha_gedola_lechumra(get_sunrise($day),get_sunset($day));
			$day->{to_print}->{"Mincha gedola"} = $mincha_gedola_lechumra;
			get_earliest_mincha($day);
		}
	}
}
...
#            $day->{to_print}->{Shiurim} = undef;
         ...
         if ($idx > 0)
         {
            ...
            #  In 2022, they said remove all but the first
            my $maariv = $day->{to_print}->{Maariv};
            $maariv =~ /(\d\d?:\d\d?)/;
            my $first_maariv = $1;
#            $maariv =~ s/\/\d\d?:\d\d?$//;     # if needed again, fix for new separator
            $day->{to_print}->{Maariv} = $first_maariv . $only . $BOLD;
            $day->{to_print}->{Sunset} = get_sunset($day);
		...
		# which just says "after each minyan".
            ...
            $siyum_day->{to_print}->{"Siyum"} = $siyum;
            ...
            if ($day->{day_of_week} eq "Sat")
            {
               ...
               $day->{to_print}->{Shacharis} = "7:00" . $only . $BOLD;
               delete $day->{to_print}->{"Chumash Shiur"};
               $burn_chometz_day->{to_print}->{"Burn Chometz"} = $burn_chometz;	# from R' Yehoshua Felder in 5781, shouldn't say "before" because it's only preferable, not required
               delete $burn_chometz_day->{to_print}->{"Burn Chometz Before"};
            }
	...
	# So we list it on this day uniquely at 8:15
         ...
         for ($i = 3; $i < 7; $i++)
         {
            ...
            elsif ($day->{day_of_week} ne "Sun"
                        # && ($holiday_shacharis_no_change || !defined($day->{public_holiday}))
						)
            {
			...
			my $mincha = $day->{to_print}->{Mincha};
			if ($day->{day_of_week} !~/Fri/ && is_during_DST($day) && $mincha !~/2\:/)
			{
				$day->{to_print}->{Mincha} = get_earliest_mincha($day) . $separator . $mincha;
			}
	...
	delete $isru_day->{to_print}->{"Likras Shabbos for Boys"};
         ...
         if (defined ($start_date))
         {
            ...
            for ($idx = find_hebrew_date($year,"Nisan",$start_date); $idx < $first_day_of_pesach_idx; $idx++)
            {
				...
				my $mincha = $day->{to_print}->{Mincha};
				if ($day->{day_of_week} !~/Fri|Sat/ && is_during_DST($day) && $mincha !~/2\:/ && $idx ne $first_day_of_pesach_idx-1)
				{
					$day->{to_print}->{Mincha} = get_earliest_mincha($day) . $separator . $mincha;
				}
...
#                }
	...
	add_late_shacharis($day);
	my $mincha = $day->{to_print}->{Mincha};
	if ($day->{day_of_week} !~/Fri|Sat/ && is_during_DST($day) && $mincha !~/2\:/)
	{
		$day->{to_print}->{Mincha} = get_earliest_mincha($day) . $separator . $mincha;
	}
	if ($day->{day_of_week} eq "Thu")
...
##         }
#
         # We appear to add (an 8:30 minyan) 2:00 Mincha until the Shabbos after Pesach.
...
#            # if ($old_shach !~ /8:30/ && !defined ($list_of_days[$idx]->{to_print}->{"Siyum"}))
...
sub handle_rosh_hashana_and_slichos
{
   ...
   foreach my $year (keys (%hebrew_years))
   {
	...
	my $first_slichos_day = $list_of_days[$slichos_idx];
	my $slichos_night = $list_of_days[$slichos_idx -1];
	if ($first_slichos_day->{day_of_week} ne "Sun")
	{
		croak "First day of Slichos not on Sunday?!";
	}
	
	# First Sunday slichos is special.
	# Calculate Chatzos
	#  We want chatzos halayla, not chatzos hayom, 
	# so calculate shaa from sunset to the sunrise of the next day.
	my $sunset = get_sunset($slichos_night);
	my $sunrise = get_sunrise($first_slichos_day);
	my $shaa = minutes_per_shaa($sunset,$sunrise);
	my $chatzos = add_minutes($sunset,$shaa*6);
	
	# From discussion with R' Yehoshua Felder on 8 Cheshvan 5782:
	my $first_slichos_time;
	if (is_earlier("1:16", $chatzos))
	{
		$first_slichos_time = "1:10 a.m.";
	}
	elsif (is_earlier("1:13", $chatzos))
	{
		$first_slichos_time = "1:05 a.m.";
	}
	else
	{
		$first_slichos_time = "1:00 a.m.";
	}
	warn "Check apparent inconsistency of Chatzos time on Slichos night.\n".
		"1:05 on Sept. 4 (2010) but 1:10 on Sept. 5 (2015, 2026)\n";
	
	#  The first Slichos is labelled on Saturday night as well as Sunday.
	$slichos_night->{to_print}->{"Melava Malka Hisorerus"} = $pre_slichos_event;  # added in 2012
	$slichos_night->{to_print}->{Slichos} = $first_slichos_time;
	$first_slichos_day->{to_print}->{Slichos} = $first_slichos_time;

#         if ($list_of_days[$slichos_idx]->{day_of_week} eq "Sun")
#         {
#            # Print 1:00 a.m. for both Sat. night and Sun. morning.
#		warn "Check Slichos time for late chatzos as in 2021!\n\n";
#            if ($slichos_idx > 1)
#            {
#               $list_of_days[$slichos_idx-1]->{to_print}->{"Melava Malka Hisorerus"} = $pre_slichos_event;  # added in 2012
...
#               my $shacharis = $day->{to_print}->{Shacharis};
               ...
               # (can't be a public holiday because erev YK can't fall on Monday, and the only holidays in Sept./Oct. are on Mondays)
               $day->{to_print}->{Slichos} = ($day->{day_of_week} eq "Sun") ?
...
#               my $shacharis = $day->{to_print}->{Shacharis};
               ...
               elsif (defined($day->{public_holiday}) && !$holiday_shacharis_no_change)
               {
               ...
               elsif ($slichos_idx == $first_day_idx+2)	
               {
                  # starting in 2022, distinguishing time on Tzom Gedalia from subsequent days
                  $day->{to_print}->{Slichos} = $slichos_tzom_gedalia;
               }
               else
               {
                    # Weekday after Tzom Gedalia
...
#         if (!$renovation)
		...
		# In 2014 Anita Miller said the tea will be at 4:45.
         ...
         # AB added:
         if ($day->{day_of_week} eq "Sun"
                    #|| (defined($day->{public_holiday}) && !$holiday_shacharis_no_change)
					)
         {
         ...
         else
         {
            ...
            $day->{to_print}->{Shacharis} = $early_shacharis . $separator . $vasikin . $breakable_separator . "7:30" . $separator . "8:30" . $BOLD;
...
#                $day->{to_print}->{Shacharis} =~ s/^6:\d\d?\//6:30\//;     # if needed again, fix for new separator
         ...
         # Apparently Vasikin was added to Simchas Torah in 5769, but removed in 5782.
         if ($vasikin_on_simchas_torah)
         {
	...
	{
		$simchas_torah_day->{to_print}->{Shacharis} = $simchas_torah_shacharis . $only;
	}
...
#         }
         ...
         while ($list_of_days[$idx]->{day_of_week} ne "Fri")
         {
			$day = $list_of_days[$idx];
			my $mincha = $day->{to_print}->{Mincha};
			if ($day->{day_of_week} !~/Fri/ && is_during_DST($day) && $mincha !~/2\:/)
	...
	if ($day->{day_of_week} =~ /Thu|Sun/)
	{
		...
		add_late_shacharis($day);
         ...
         for ($i = 1; $i < 5; $i++)
         {
            ...
            add_late_shacharis($day);
			my $mincha = $day->{to_print}->{Mincha};
			if ($day->{day_of_week} !~/Fri/ && is_during_DST($day) && $mincha !~/2\:/ && $i ne 1)
			{
				$day->{to_print}->{Mincha} = get_earliest_mincha($day) . $separator . $mincha;
			}
#            my $old_shach = $day->{to_print}->{Shacharis};
...
sub handle_yom_kippur
{
   ...
   foreach my $year (keys (%hebrew_years))
   {
	...
	delete $erev->{to_print}->{"Likras Shabbos for Boys"};
         ...
         $day->{to_print}->{Neila} =
            "(approx.) " . round_down_to_5_minutes(subtract_minutes($sunset,45)); # requested in 2023; this is 1.5 hours before Motzoai Yom Kippur
...
#        $second_day->{to_print}->{"Neilas Hachag"} = "after Mincha";	# no longer printing this as of 2023
    }
...
sub add_drasha($)
{
    ...
    my $mincha = $BOLD . $shabbos_mincha_gedola . $separator . $early_mincha . $separator . $main_mincha;
    $day->{to_print}->{Drasha}  = $drasha;
    $day->{to_print}->{Mincha} = $mincha;
...
#    print "$day->{Eyear}/$day->{Emon}/$day->{Eday} new value was " . $day->{to_print}->{"Fast begins"} . "\n";
		...
		my $sunset = get_sunset($day);
		$day->{to_print}->{Sunset} = $sunset;
		my $primary_mincha = $BOLD . round_down_to_5_minutes(subtract_minutes($sunset,30));
		###   tweaking in 2023:
		my $earliest_mincha = get_earliest_mincha($day);	# 1:00/1:05/2:00/2:05
		my $mincha;
		if ($day->{day_of_week} =~/Fri/)
		{
			$mincha = $primary_mincha . $only;
		}
		elsif ($name =~ /$sabt/)
		#	(is_earlier(add_minutes($even_earlier_weekday_mincha, 30), $primary_mincha))	#	POSSIBLE ALTERNATIVE
		{
			$mincha = $earliest_mincha . $separator . $even_earlier_weekday_mincha . $separator . $primary_mincha;
		}
		else
		{
			$mincha = $earliest_mincha . $separator . $primary_mincha;
		}
		...
		$day->{to_print}->{Mincha} = $mincha;
         ...
         if ($day->{day_of_week} ne "Fri")  # AB added this condition
         {
			...
			# Instructions as of 2023:  Add 9:30 Maariv at the end of all fast days (I guess not 17 Tammuz and 9 Av)
			#  but not the newer additional ones (8:15 and 9:45).
			#  In the case of erev Purim it will get overridden later,
			#  but it's useful for the case where Taanis Esther is not erev Purim.
			if ($name =~ /$abt|$te|$tg/)
			{
...
#            if (($name eq $abt && ($day->{day_of_week} ne "Sun"
#                        && !defined($day->{public_holiday}))
#            			# $day->{to_print}->{Shacharis} =~ $fast_day_shacharis
#		)
...
#               )
#            {
#                $maariv .= $separator . "8:15" . $breakable_separator . "9:30" . $separator . "9:45";
#            }
#            else
#            {
#                $maariv .= "$BOLD$only";
...
#         add_notes($day,"***For extenuating circumstances please consult the Rav");	# removing this in 2023
      }
   }
}
...
#      }
      ...
      $erev->{to_print}->{"Megillas Esther"} = $megilla;
		remove_notes($erev, "Tehillim after late Maariv");
	warn "In 2023 there was a third evening Megilla reading added.";
...
#         }
         ...
         # Modifying again in 2022, getting rid of this distinction - always have 7:30 in addition, even if vasikin starts late

#	elsif (is_earlier($vasikin, "7:00"))
	else
	{
		# Vasikin will begin 6:35 or later, so we add the 6:30 first.
		# But we add a 7:30 as well, because vasikin is significantly earlier.  ALWAYS HAVE 7:30 (since 2022)
		# Not sure whether this is the right cutoff.
		$day->{to_print}->{Shacharis} = 
				$BOLD."6:30".$separator . $vasikin . $breakable_separator . "7:30" . $separator . "8:30";
		$day->{to_print}->{"Megillas Esther "} =
				"7:10" . $separator . $megilla_vasikin . $breakable_separator . "8:10" . $separator . "9:10";
...
#         }
      ...
      $day->{to_print}->{"Yeshivas Mordechai HaTzaddik"} = "10:00". $endash . "11:00";  # Check this? - for several years we were listing this as 10:15

      if ($day->{day_of_week} ne "Fri")
      {
         $day->{to_print}->{Mincha} = get_earliest_mincha($day) . $separator . $purim_mincha; # added 1:00 Mincha in 2018; they want 2:00 since 2022
   ...
   warn "Replace hyphen with en-dash and make sure it's non-breaking, on Purim.\n\n";
}
...
sub handle_chanukah
{
   ...
   #	Now in 2023 that there are more Maarivs
   ...
   foreach my $year (keys (%hebrew_years))
   {
	...
	if ($erev->{to_print}->{Mincha} !~/$early_shabbos_mincha/)
	{
		$erev->{to_print}->{Mincha} = $shabbos_no_ss_early_mincha . $separator . $mincha . $BOLD; # forgot bold in 2019
         ...
         $erev->{to_print}->{Maariv} =
            round_up_to_5_minutes(add_minutes($sunset,82));	# they said it was too early in 2021
      ...
      $day->{to_print}->{Mincha} = $BOLD . get_earliest_mincha($day) . $separator . "4:30" . $separator . $later_mincha;   # 4:30 added in 2010
      $day->{to_print}->{Maariv} =
...
sub list_misc_days()
{
   ...
   foreach my $year (keys (%hebrew_years))
   {
      ...
      if ($day_idx)
      {
         ...
         ####   FROM 5782 ONWARDS WE ARE NO LONGER MOVING MINCHA EARLY ON THIS OCCASION.
#         while ($day_idx > 0 && $list_of_days[$day_idx]->{day_of_week} ne "Sat")
#         {
#            $day_idx--;
#         }
#         if ($day_idx > 0 && !$renovation  && !$COVID)
#         {
#            $day = $list_of_days[$day_idx];
...
sub add_molad(@)
{
    ...
    # It keeps only integer seconds so we have to add a second sometimes:   ### MAYBE NOT!!!!  It's keeping the fractional part as well --- until we switched to Strawberry Perl in 2020, then had to reinstate this hack....?
...
#        print "ARE WE HERE YET?";
        if ($kl_index)
        {
            ...
            my $kl_note;
            # choosing 36 minutes after sunset as the earliest deadline to list on a given day;
            # otherwise list it the previous night
            if ($kiddush_levana[3] >= 17 && is_earlier(add_minutes(get_sunset($kl_day),36),$kl_time))
            {
            ...
            else
            {
                ...
                if ($kiddush_levana[3] == 0 ||
                    ($kiddush_levana[3] < 8 && is_earlier($kl_time, subtract_minutes(get_sunrise($kl_day),36))))
                {
            ...
            if ($kl_index)  # need this condition in case subtracted 1 above
            {
                ...
                # If it's very early on Saturday night, possibly even before Shabbos ends....
                elsif ($kl_day->{day_of_week} =~ "Sat" && $kiddush_levana[3] >= 17 && 
                	is_earlier(add_minutes(get_sunset($kl_day),36),$kl_time) && 
                	is_earlier($kl_time, add_minutes(get_sunset($kl_day),70))  &&  # enough time for Maariv?
                	$kl_index>2)
                {
                	add_notes($list_of_days[$kl_index-2], $BOLD . "Last ideal night for Kiddush Levana");
                }
    ...
    # May 2021 eclipse - U1 right at sunrise

	handle_eclipse(2021,11,18);
	# May 2022 eclipse after deadline
	handle_eclipse(2022,11,7);
	# Oct 2023 not visible here
	handle_eclipse(2024,9,17);
	handle_eclipse(2025,3,13);
	# Sept 2025 not visible here
	handle_eclipse(2026,3,2);
	# August 2026 eclipse after deadline
	# Jan 2028 after deadline
	# July 2028 not visible here
	# Dec 2028 not visible here
	handle_eclipse(2029,6,25);	# U1 is at 9:32 p.m., so there really isn't time, should this be moved to the previous night?
	warn "The eclipse on 2029 June 25 starts (U1) at 9:32 p.m. EDT; is the instruction misleading?\n\n";	
	# Dec 2029 after deadline

    warn "Lunar eclipses considered only until 2029.  Beyond then, Kiddush Levana is calculated based on the molad only." .
        " The deadline may be earlier if there is a lunar eclipse.\n\n";
        # See https://en.wikipedia.org/wiki/List_of_21st-century_lunar_eclipses
...
#                    $day->{to_print}->{"Women's Avos"} = "#" . $perek . " " . $womens_avos;
                    $day->{to_print}->{"Women's Avos"} = $womens_avos;		# for 5784 moving the perek number out of this listing.
...
#    }
	...
	# if adding more here, use $endash instead of hyphen
    );
...
#        [2012,3,17],       # DONE!!!
        [2014,3,15],	# Erev Purim
	[2014,12,20],	# Miketz - Chanuka
	[2015,4,18],	# Shmini - Birkas HaChodesh
	[2015,5,9],		# Emor - Omer 35
	[2015,12,12],	# Miketz - Chanuka
	[2016,6,11],	# Bamidbar - Omer 49
	[2016,8,13],	# Devarim - Erev Tisha B'Av
	[2016,8,20],	# Vaeschanan - Nachamu
#	[2017,3,11],
	[2017,12,16],	# Miketz - Chanuka
	[2018,7,21],	# Devarim - Erev Tisha B'Av
#	[2024,3,23],	# Vayikra—Zachor - Erev Purim, as a test
    );
    ...
    foreach my $date (@combine_prenotes)
    {
        ...
        if ($idx)
        {
            $list_of_days[$idx]->{prenotes} =~ s/,/ $emdash /;
        }
    }
...
sub is_earlier ($$)
{
   ...
   {
      $second_hr = $1;
	  $second_hr =~ s/12/0/;		# hack for comparing Mincha Gedola; I hope it doesn't break anything else
      $second_min = $2;
   }
   else
   {
...
sub dump_excel_calendar()
{
   ...
   warn "Remember to set Letter-size paper when formatting in Excel (rather than A4).\n";
...
#        );
    ...
    #  OLD VERSION: don't need the SaveAs command because the destructor will automatically prompt to save the file.
    ...
    ### NOW IN 2020 WE NEED THE SaveAs COMMAND on my new computer!
...
sub print_half_month_to_excel ($$$$)
{
   ...
   my $start_day = $list_of_days[$start_idx];
   my $col = 1;
   my $row = 1;
   ...
   # starting for 5784 we will do this on every page to help prevent confustion in case pages are printed separately.
...
#   }
   ...
   #### MAY NEED TO MANUALLY RESIZE BOTTOM ROW!!!!!
    ...
    if ($start_day->{Eday} > 1 # && $COVID
    )
    {
    	$footnote_text = $footnote_text . $change_note . chr(10) . chr(10);		# In 5781 this started as a COVID note
		warn "May need to resize bottom row manually because of YMM note or change note!";
    }
    ...
    if ($footnote_text ne "")
    {
      if ($col>3)
      {
            $row++;
            $end_row_of_current_date++;
            merge_range($sheet, $row, 4, $row+6, 7);
      }
      else
      {
            merge_range($sheet, $row, 4, $row+2, 7);
      }
...
sub set_up_for_printing ($$$)
{
   ...
   $sheet->Columns("B:B")->{ColumnWidth} = $narrow_width;
   $sheet->Columns("C:C")->{ColumnWidth} = $narrow_width;
   $sheet->Columns("D:D")->{ColumnWidth} = $narrow_width;
...
#   }
   ...
   elsif ($sheet->{Name} =~ /2021_2_2/)
   {
        $sheet->Columns("A:A")->{ColumnWidth} = 16.29;
        $sheet->Columns("B:B")->{ColumnWidth} = 16.29;
        $sheet->Columns("C:C")->{ColumnWidth} = 16.29;
        $sheet->Columns("D:D")->{ColumnWidth} = 16.29;
        $sheet->Columns("E:E")->{ColumnWidth} = 19.00;
        $sheet->Columns("F:F")->{ColumnWidth} = 22.00;
   }
   elsif ($sheet->{Name} =~ /2021_3_1/)
   {
        $sheet->Columns("A:A")->{ColumnWidth} = 18.43;
        $sheet->Columns("B:B")->{ColumnWidth} = 17.00;
        $sheet->Columns("C:C")->{ColumnWidth} = 17.00;
        $sheet->Columns("D:D")->{ColumnWidth} = 17.00;
        $sheet->Columns("E:E")->{ColumnWidth} = 17.00;
   }
   elsif ($sheet->{Name} =~ /2022_3_1/)
   {
        $sheet->Columns("A:A")->{ColumnWidth} = 17.00;
        $sheet->Columns("B:B")->{ColumnWidth} = 17.00;
        $sheet->Columns("C:C")->{ColumnWidth} = 17.00;
        $sheet->Columns("D:D")->{ColumnWidth} = 20.00;
        $sheet->Columns("F:F")->{ColumnWidth} = 20.00;
        $sheet->Columns("G:G")->{ColumnWidth} = 27.00;
   }
   elsif ($sheet->{Name} =~ /2024_3_2/)
   {
        $sheet->Columns("A:A")->{ColumnWidth} = 18.86;
        $sheet->Columns("B:B")->{ColumnWidth} = 17.22;
        $sheet->Columns("C:C")->{ColumnWidth} = 17.22;
        $sheet->Columns("D:D")->{ColumnWidth} = 17.22;
        $sheet->Columns("E:E")->{ColumnWidth} = 17.22;
   }
   elsif ($sheet->{Name} =~ /2024_10_1/)
   {
        $sheet->Columns("B:B")->{ColumnWidth} = 17.44;
        $sheet->Columns("C:C")->{ColumnWidth} = 17.44;
        $sheet->Columns("E:E")->{ColumnWidth} = 17.78;
   }


	warn "We should resize the row heights so that the calendar fills up the page.  For winter 5781 I did this manually. \n";
